...
~> `passbolt_folders` decrypts v5 folder metadata when the authenticated user has access to the folder metadata key, so `name` and `path` work for both v4 and v5 folders.
